...
for (let i = 0; i < 9; i++) {
  ...
  gamePieces.push(element);
}

const EMPTY_GAME_PIECE = null;
const gameState = [
  [gamePieces[0], gamePieces[1], gamePieces[2]],
  [gamePieces[3], gamePieces[4], gamePieces[5]],
  [gamePieces[6], gamePieces[7], EMPTY_GAME_PIECE],
];
  ...
  gameState.forEach((row, rowIndex) => {
    row.forEach((tile, columnIndex) => {
      if (tile) {
        ...
        tile.style.left = `${columnIndex * 100}px`;

        tile.style.backgroundPosition = `-${columnIndex * 100}px -${
          rowIndex * 100
        }px`;
